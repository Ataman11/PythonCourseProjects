...
class Ship:
    ...
    def inc_angle_vel(self, acc):
        self.angle_vel += acc
        
    def dec_angle_vel(self, acc):
        self.angle_vel -= acc
    ...
    def shoot(self):
        ...
        forward_vector = angle_to_vector(self.angle)
        vel = [0, 0]
        vel[0] = self.vel[0] + 5 * forward_vector[0]
        vel[1] = self.vel[1] + 5 * forward_vector[1]
        pos = [0, 0]
...
def keydown(key):
    if key == simplegui.KEY_MAP['right']:
        my_ship.inc_angle_vel(0.1)
    elif key == simplegui.KEY_MAP['left']:
        my_ship.dec_angle_vel(0.1)
    if key == simplegui.KEY_MAP['up']:
        my_ship.thrust = True
        my_ship.play_thrust_sound()
